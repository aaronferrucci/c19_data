...
source("fontsize.R")
plotit <- function(data, title, dperc=F) {
  ...
  dpos <- append(-diff(pos), NA)
  dneg <- append(-diff(neg), NA)
  ...
  rate <- data.frame(
    date=data[data$result=="positive",]$date,
    rate=pos / (pos + neg),
    t = "cumulative %"
  )
  if (dperc) {
  ...
  p2 <- p1 +
    geom_line(data=rate, mapping=aes(x=date, y=rate*yscale, color=t)) +
    geom_point(data=rate, mapping=aes(x=date, y=rate*yscale, color=t)) +
    labs(color = "") +
